...
/* ── 验证支付（供结果页调用）────────────────────── */
async function handlePaymentReturn(chartHashOrSession, lang, onProgress, isSession) {
  const maxAttempts = isSession ? 3 : 8; // session模式重试少，因为几乎即时
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    if (onProgress) onProgress(attempt, maxAttempts);
    try {
      // isSession=true 时用 sessionKey，否则用 chartHash
      const body = isSession
        ? { sessionKey: chartHashOrSession, lang }
        : { chartHash: chartHashOrSession, lang };
      ...
      if (res.status === 402 && attempt < maxAttempts) {
        const waitSec = isSession ? 2 : attempt * 2;
...
function checkPaymentReturn() {
  ...
  // session 模式（Gumroad）或 hash 模式（旧版）都支持
  if (!sessionKey && !hash) return null;
  return {
    sessionKey:  sessionKey || null,
    chartHash:   hash || null,
    lang:        p.get('lang') || 'en',
    productType: p.get('type') || 'reading',
    element:     p.get('element') || null,
    origHash:    p.get('origHash') || null
  };
}
function cleanUrl() {
